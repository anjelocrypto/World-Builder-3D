...
- **Mountain ring** (`shared/elevation.ts` + `ROAD_ELEVATION_PROFILES` / `MOUNTAIN_ROAD_IDS` / `MOUNTAIN_MASSIFS` / `MOUNTAIN_REAL_LIGHTS` in `cityData.ts`): 8 mountain roads (`mountain-switchbacks`, `mountain-lookout`, `ridge-east`, `ridge-east-high`, `ridge-west`, `summit-pass`, plus the east/west foothill ramparts `ridge-east-far` and `ridge-west-far` that hug the outer-loop east/west legs) carry per-vertex elevation profiles in `ROAD_ELEVATION_PROFILES` (Y up to 22m at the observatory summit). `getRoadElevationAt(x,z)` projects onto the nearest mountain segment and lerps Y, smoothstepping to 0 over an 8m blend-out shoulder; `getVehicleGroundY` is the call site used by `LocalPlayer` (vehicle drive, **walking ground**, and **safe-exit Y** so the player on foot stands on the slope instead of clipping through an elevated road) and `AmbientTraffic` so the player car and the 5 ambient mountain cars (3 routes — `ridge-east-high`, `ridge-west`, `summit-pass`) climb the slopes instead of clipping through them. `RegionalRoadSegment` builds an explicit (right, fwd, up) Matrix4 basis so each road quad sits at the segment midY tilted along its 3D tangent (flat segments collapse to the previous y=0.005 behaviour). Lamp Y is offset by `getRoadElevationAt(lamp.x, lamp.z)` for mountain-style lamps; the **mountain-area `cliff_wall` and `guardrail`** in `ObstacleMesh` also sample the elevation helper so the wall/rail rides up the slope with the carriageway. 15 cosmetic `MOUNTAIN_MASSIFS` cones surround the city — north (5), east (3 at x=497), west (3 at x=-497), and south (4 at z=495 outside the village/cabins) — and 6 `MOUNTAIN_REAL_LIGHTS` mark the summit, ridge peaks, and switchback junctions. `BiomeGround` paints darker mountain strips at the east/west/south world edges to ground the new ramparts. Parked mountain cars car-14..17 carry pre-computed Y values matching the road profile (mirrored in `artifacts/api-server/src/socket/cityData.ts`). Validator: `mountainRing OK: roads=8, profiledVerts=36, maxGrade=13.3%, summitY=22m, endpointMaxGap=0.00m, gradeViolations=0, endpointViolations=0, traffic=22/22, lamps=37, realLights=6, massifsClear=15/15, roadSides={n:4,e:2,s:0,w:2}, massifSides={n:3,e:4,s:4,w:4}, parkedExits=4/4, obstacleSlope=15/15` — enforces per-vertex profile length match, ≤0.18 max grade, ≤0.5m endpoint Y continuity at shared graph nodes, mountain traffic waypoints on a mountain carriageway, massif clearance vs every road, side-coverage counts (proving the city is surrounded — south stays road-free to preserve the forest village), parked-mountain-car Y matches the sampled road profile (proves the player exit ground sample is correct), and mountain `guardrail`/`cliff_wall` Y is consistent with the nearby road profile.
