...
- **HUD (in-game UI):** Redesigned in `game/HUD.tsx` to feel like a polished open-world driving game. Top-left cluster: username panel + connection (`Wifi`/`WifiOff` lucide icon, ONLINE/OFFLINE) + player count (`Users` icon), with a phase-tinted clock chip directly underneath. Top-right: 200px minimap with a phase-tinted frame, `Compass` icon, world coordinates, N/E/S/W compass labels, and a directional **player arrow** (heading derived from successive position deltas in a `useRef` — no React state). Bottom-left: segmented 10-tile **health bar** with `Heart` icon and color states (green > 50, amber > 25, red ≤ 25) plus per-segment glow. Bottom-right (vehicle only): SVG **speedometer** with 270° arc gauge, color-shifting tick (green → cyan → amber → red), `Car`/`Gauge` icons, and the vehicle label. Top-center: subtle pill-shaped controls hint. Bottom-center: interaction prompt rendered as a card with a key-cap **`E`** and the action text. All panels share one chassis (`PANEL_BG`/`PANEL_BORDER`/`PANEL_RADIUS`/`PANEL_SHADOW` constants) with `backdrop-filter: blur(6px)`. HUD root keeps `pointer-events: none`. All existing HUD props/behavior preserved (`health`, `speed`, `inVehicle`, `showInteract`, `vehicleLabel`, `playerCount`, `username`, `connected`, `clockLabel`, `clockPhase`, `raceActive/raceTime/racePassed`, `playerPositionX/Z`).
