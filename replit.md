...
- 9 `REGIONAL_ROADS` polylines (typed `asphalt | bridge | forest | mountain | dirt`) connect biomes to the central grid.
...
- 12 ambient NPC pedestrians on sidewalk loops + 10 ambient AI cars on **4 traffic routes** (city loop, bridge-forest spine, mountain switchbacks, outer service ring) — all deterministic from `Date.now()`, client-only.
